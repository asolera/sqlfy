...
const configExists = () => fs.existsSync(SQLFY_CONFIG_PATHFILE) ? true : false;

const isConfigValid = config => (config.sourceDir && config.destDir && config.templateExtension) ? true : false;
...
const save = (data, filePath, sourceDir, destDir) => {
    let destFilePath = filePath.replace(sourceDir, destDir);
...
const convert = (filePath, config) => {
    ...
    fs.readFile(filePath, 'utf8', (error, data) => {
        ...
        data = eval('`'+data+'`');
        save(data, filePath, config.sourceDir, config.destDir);
    });
}
